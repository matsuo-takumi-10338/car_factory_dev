...
def gld_bi_mom_factory():
    ...
    summary_df = (
        slv_df.withColumn("work_date", F.to_date(F.col("timestamp")))
        .groupBy("work_date", "factory_id", "line_id", "process_code", "car_model_code")
        .agg(
            F.count(F.col("vin")).alias("total_production_cnt"),
            F.sum(F.when(F.col("process_result_code") == 1, 1).otherwise(0)).alias(
                "good_production_cnt"
            ),
            F.sum(F.when(F.col("process_result_code") != 1, 1).otherwise(0)).alias(
                "defect_production_cnt"
            ),
    ...
    final_bi_df = (
        summary_df.join(
            m_factory.select("factory_id", "factory_name"), on="factory_id", how="left"
        )
        .join(
            m_process.select("process_code", "process_disp_name", "sort_order"),
            on="process_code",
            how="left",
        )
        .join(
            m_car_model.select("car_model_code", "car_model_name", "powertrain_type"),
            on="car_model_code",
            how="left",
        )
    )
...
def gld_api_vehicle_trace():
    ...
    joined_df = (
        slv_df.join(
            m_factory.select("factory_id", "factory_name", "location_name"),
            on="factory_id",
            how="left",
        )
        .join(
        ...
        .join(
            ...
            on=["car_model_code", "process_code"],
            how="left",
        )
    )
...
def gld_api_operator_status():
    ...
    joined_df = (
        ...
        .join(
            m_process.select("process_code", "process_disp_name"),
            on="process_code",
            how="left",
        )
        .join(
            m_car_model.select("car_model_code", "car_model_name"),
            on="car_model_code",
            how="left",
        )
        .join(
            m_production_standard.select(
                "car_model_code", "process_code", "target_cycle_time"
            ),
            on=["car_model_code", "process_code"],
            how="left",
        )
